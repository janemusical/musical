...
def createEvent (r, student):
    ...
    if student != "Complete": #only truncate student name if not doing the Complete Schedule
        studentLengthNumber = len(student) 
        studentFirstName = student[:(studentLengthNumber - 2)]
    else:
         studentFirstName = student
         
    event.add('summary', 'Chitty Chitty Bang Bang - ' + studentFirstName) #TODO hardcoded name of production (put as cell in Spreadsheet that is read in)
    desc = ''
    ...
    event['location'] = vText(r.where) #TODO
...
def generateRehearsals (sheet, ScenestoChars, CharstoStudents):
    ...
    for r in schedule:
        ...
        #find date and time
        (date,st,et,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,typ1,typ2,typ3,typ4,typ5,typ6,typ7,typ8,typ9,typ10,typ11,typ12,loc,x) = r

        start_dt = createTimes (date, st)
        end_dt   = createTimes (date, et)

        #create list of scenes or characters
        scenes_tmp = (s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12)
        scenes = []
        for s in scenes_tmp:
            if s != "":
              scenes.append(s)
        ...
        rehersal_type_tmp = (typ1,typ2,typ3,typ4,typ5,typ6,typ7,typ8,typ9,typ10,typ11,typ12)
        types = []
...
def generateScenestoChars (sheet):
    ...
    for r in scenes:
        ...
        (scene, act, sceneNumber, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, x) = r
...
for student in Students:
    ...
    filename = 'Chitty Chitty Bang Bang - ' + student + ".ics"
