...
def createEvent (r, student):
    ...
    event.add('summary', 'Pippin - ' + studentFirstName) #TODO hardcoded name
    desc = ''
    ...
    organizer.params['name'] = vText('Chaminade') #TODO
...
def generateRehearsals (sheet, ScenestoChars, CharstoStudents):
    ...
    for r in schedule:
        ...
        rehersal_type_tmp = (typ1,typ2,typ3,typ4,typ5)
        types = []
...
def generateScenestoChars (sheet):
    ...
    for r in scenes:
        ...
        (scene, act, c1, c2, c3, c4, c5, c6, c7, x) = r
...
def generateCharstoStudents (sheet):
    ...
    for r in castAssignments:
        ...
        if not r:
            break
        #print (r)
        (student, c1, c2, c3, c4, c5, c6,x) = r
        Students.append(student)
        chars_tmp = (c1, c2, c3, c4, c5, c6)
        chars = []
        for c in chars_tmp:
            if c != "":
              chars.append(c)
...
sheet = file.open("MusiCal: Pippin - Chaminade") #open sheet
...
cal = StudentCalendars[student]
filename = 'Pippin - Complete Schedule.ics'
f = open(os.path.join(directory, filename), 'wb')
f.write(cal.to_ical())
f.close()
