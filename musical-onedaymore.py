...
def createEvent (r, student):
    ...
    event.add('summary', 'One Day More - ' + studentFirstName) #TODO hardcoded name
    desc = ''
...
def generateScenestoChars (sheet):
    ...
    for r in scenes:
        ...
        (scene, act, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, x) = r

        #create list of characters and ensembles

        chars_tmp = (c1,c2,c3, c4, c5, c6, c7, c8, c9, c10, c11, c12)
        chars = []
        for c in chars_tmp:
            if c != "":
              chars.append(c)
...
sheet = file.open("MusiCal: One Day More - ACTT") #open sheet
...
for student in Students:
    ...
    filename = 'One Day More - ' + stud + ".ics"
...
cal = StudentCalendars[student]
filename = 'One Day More - Complete Schedule.ics'
f = open(os.path.join(directory, filename), 'wb')
f.write(cal.to_ical())
f.close()
