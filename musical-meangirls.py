...
def createEvent (r, student):
    ...
    event.add('summary', 'Mean Girls - ' + studentFirstName) #TODO hardcoded name
    desc = ''
    ...
    organizer.params['name'] = vText('Visitation Academy') #TODO
...
def generateRehearsals (sheet, ScenestoChars, CharstoStudents):
    ...
    for r in schedule:
        ...
        #find date and time
        (date,st,et,s1,s2,s3,s4,s5,s6,s7,s8,typ1,typ2,typ3,typ4,typ5,typ6,typ7,typ8,loc,x) = r

        start_dt = createTimes (date, st)
        end_dt   = createTimes (date, et)

        #create list of scenes or characters
        scenes_tmp = (s1,s2,s3,s4,s5,s6,s7,s8)
        scenes = []
        for s in scenes_tmp:
            if s != "":
              scenes.append(s)
        ...
        rehersal_type_tmp = (typ1,typ2,typ3,typ4,typ5,typ6,typ7,typ8)
        types = []
...
def generateScenestoChars (sheet):
    ...
    for r in scenes:
        ...
        (scene, act, sceneNumber, c1, c2, c3, c4, c5, c6, c7,c8, c9, c10, c11, c12, c13, c14, c15, c16, c17, c18, c19, x) = r

        #create list of characters and ensembles

        chars_tmp = (c1,c2,c3, c4, c5, c6, c7,c8, c9, c10, c11, c12, c13, c14, c15, c16, c17, c18, c19)
        chars = []
        for c in chars_tmp:
            if c != "":
              chars.append(c)
...
def generateActDialogueLists(sheet):
    ...
    CharactersAndEnsembles = sheet.worksheet("Characters and Ensembles").get_all_values()
...
def generateCharstoStudents (sheet):
    ...
    for r in castAssignments:
        ...
        if not r:
            break
        #print (r)
        (student, c1, c2, c3, c4, c5, c6,c7, x) = r
...
sheet = file.open("MusiCal: Mean Girls - Viz") #open sheet
...
#
    if "Dialogue" in scene and "***" not in scene:
...
for student in Students:
    ...
    filename = 'Mean Girls - ' + student + ".ics"
...
cal = StudentCalendars[student]
filename = 'Mean Girls - Complete Schedule.ics'
f = open(os.path.join(directory, filename), 'wb')
f.write(cal.to_ical())
f.close()
