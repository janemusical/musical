...
def createEvent (r, student):
    ...
    event.add('summary', 'Matilda - ' + studentFirstName) #TODO hardcoded name
    desc = ''
    ...
    organizer.params['name'] = vText('De Smet') #TODO
...
def generateRehearsals (sheet, ScenestoChars, CharstoStudents):
    ...
    for r in schedule:
        ...
        rehersal_type_tmp = (typ1,typ2,typ3,typ4)
        types = []
...
sheet = file.open("MusiCal: Matilda - Shooting Star Productions") #open sheet
...
cal = StudentCalendars[student]
filename = 'Matilda - Complete Schedule.ics'
f = open(os.path.join(directory, filename), 'wb')
f.write(cal.to_ical())
f.close()
