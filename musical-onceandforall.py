...
def createEvent (r, student):
    ...
    event.add('summary', 'Once and For All - ' + studentFirstName) #TODO hardcoded name
    desc = ''
    ...
    organizer.params['name'] = vText('Arch City Kids') #TODO
...
def generateRehearsals (sheet, ScenestoChars, CharstoStudents):
    ...
    for r in schedule:
        ...
        #find date and time
        (date,st,et,s1,s2, typ1,typ2,loc,x) = r
...
sheet = file.open("MusiCal: ACTT - Once and For All") #open sheet
...
for student in Students:
    ...
    filename = 'Once and For All - ' + student + ".ics"
...
cal = StudentCalendars[student]
filename = 'Once and For All - Complete Schedule.ics'
f = open(os.path.join(directory, filename), 'wb')
f.write(cal.to_ical())
f.close()
